...
class BotPlayer(Player):
    ...
    def compute_total_damage(self, rc, cooldown):
        dmg = 0
        for tower in self.opp_towers:
            numBombTilesInRange = self.bomber_arr[tower.x][tower.y]
            numSnipeTilesInRange = self.gunship_arr[tower.x][tower.y]

            if tower.type == TowerType.BOMBER:
                dmg += 6 * ceil(numBombTilesInRange/cooldown / 15)
            elif tower.type == TowerType.GUNSHIP:
                dmg += 25 * ceil(numSnipeTilesInRange/cooldown / 20)
