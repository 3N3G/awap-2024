...
class BotPlayer(Player):
    ...
    def play_turn(self, rc: RobotController):
        print("gunship list: ", len(self.gunship_list))
        ...
        rushing = False # self.opponent_rushing(rc)
        if (rushing):
        ...
        elif (self.gunship_count > 0 and self.solar_count < 10 and hp == 2500):
            self.build_solar(rc)
        else:
    ...
    def play_given_safe(self, rc, safe, hp):
        ...
        if safe and self.gunship_count > int(1.2 * self.solar_count) and len(self.gunship_list) > 0:
            self.build_solar_or_reinforcer(rc)
