...
class BotPlayer(Player):
    def __init__(self, map: Map):
        ...
        self.path_to_map_ratio = len(map.path) / (map.height*map.width)
    ...
    def play_given_safe(self, rc, safe, hp):
        ...
        if safe and hp == 2500 and self.bomber_count > int(0.2 * self.solar_count) and len(self.gunship_list) > 0:
            ...
            if (self.bomber_count + self.gunship_count == 0):
                self.build_bomber(rc)
            if (self.bomber_count / (self.bomber_count + self.gunship_count) * 2 < self.path_to_map_ratio):
